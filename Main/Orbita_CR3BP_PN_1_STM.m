...
H = subs(subs(subs(subs(subs(subs(H,var_vec(1),x00),var_vec(2),y00),var_vec(3),z00),var_vec(4),vx00),var_vec(5),vy00),var_vec(6),vz00);
    ...
    % Inicializar STM
    STM.init = eye(length(i0));

    STM(1).vec = Vectorize(STM.init);

    options = odeset('AbsTol',1e-15,'RelTol',1e-10);

    %Integración
    [t00,p00,~] = ode113(@(t,y) CR3BP_PN_1_STM_ODE(t,y,PN_1,mu,J), [t0 tf], [i0;STM(1).vec], options);%,'Events', @(t,y) cruce(t,y,mu));
