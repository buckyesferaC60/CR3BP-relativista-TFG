...
for b = b_values_init:b_values_end
 ...
 kappa_vec = [0 1e-11 1e-6 1e-3 1e-2];
kappa = kappa_vec(b);
...
hold on
plot3(L1(1),L1(2),L1(3),'+',"Color",[kappa 0 2*mu],'MarkerSize',10);
plot3(L2(1),L2(2),L2(3),'+',"Color",[kappa 0 2*mu],'MarkerSize',10);
plot3(L3(1),L3(2),L3(3),'+',"Color",[kappa 0 2*mu],'MarkerSize',10);
plot3(L4(1),L4(2),L4(3),'+',"Color",[kappa 0 2*mu],'MarkerSize',10);
plot3(L5(1),L5(2),L5(3),'+',"Color",[kappa 0 2*mu],'MarkerSize',10);
...
x(3).init = linspace(1-mu-0., 2-mu, round(n_max));

%x(1).init = linspace(-0.3,0,n_max);

y = linspace(-2,2,n_max);
vx = linspace(-5,5,n_max);
...
if isreal(vy00)
    ...
    %Integración
    [t00,p00,te,pe,ie] = ode113(@(t,y) CR3BP_PN_1_ODE(t,y,PN_1), [t0 tf], i0, options);
